...
class GuardianController extends Controller
{
    ...
    public function store(Request $request)
    {
        $request->validate([
            ...
            'file_wali' => 'required|file|mimes:jpg,jpeg,png,pdf|max:2048'
        ]);
        $student_id = Student::where('user_id', Auth::user()->id)->value('id');
        $file = $request->file('file_wali');
        $fileName = Str::random(7) . '.' . $file->getClientOriginalExtension();
        $file->storeAs('file_wali', $fileName);
        Guardian::create([
    ...
    public function update(Request $request, $id)
    {
        ...
        $student_id = Student::where('user_id', Auth::user()->id)->value('id');
        if ($request->hasFile('file_wali')) {
            $file = $request->file('file_wali');
            $fileName = Str::random(7) . '.' . $file->getClientOriginalExtension();
            $file->storeAs('file_wali', $fileName);
        } else {
            $fileName = Guardian::find($id)->value('file_wali');
        }
        ...
        return back()->with('success', 'Data wali berhasil diubah');
    }
